...
from src.modelling.cyclegan import *
...
class Loader():
    ...
    def _my_numel(self, m, only_trainable=False, verbose=True):
        """
        Returns the total number of parameters used by `m` (only counting
        shared parameters once); if `only_trainable` is True, then only
        includes parameters with `requires_grad = True`
        """
        parameters = list(self._get_parameters_recursive(m))
        if only_trainable:
            parameters = [p for p in parameters if p.requires_grad]
        unique = {p.data_ptr(): p for p in parameters}.values()
        model_total_params = sum(p.numel() for p in unique)
    ...
    def _get_parameters_recursive(self, module):
        """
        Recursively get all parameters of a module and its sub-modules.
        """
        for name, sub_module in module.__dict__.items():
            if isinstance(sub_module, torch.nn.Parameter):
                yield sub_module
            elif isinstance(sub_module, torch.nn.Module):
                yield from self._get_parameters_recursive(sub_module)
